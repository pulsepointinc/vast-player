...
function proxyMethod(method, swallowError) {
    return function callMethod() {
        ...
        try {
            ...
            if (e.message.startsWith('VASTPlayerNotReady') || e.message.startsWith('VPAIDPlayerNotReady')) {
                return;
            }
            this.logError_(e);
            if (!swallowError) {
...
VPAIDWrapper.prototype = {
    ...
    initAd: function (width, height, viewMode, desiredBitrate, creativeData, environmentVars) {
        try {
            ...
            if (parameters.sentrydsn) {
                Sentry.init({dsn: parameters.sentrydsn, whitelistUrls: [/.*contextweb.*/]});
            }
            ...
            player.load(uri)
            .then(function () {
                player.on(VPAID_EVENTS.AdStopped, cleanupFunc);
                player.on(VPAID_EVENTS.AdSkipped, cleanupFunc);
                player.on(VPAID_EVENTS.AdVideoComplete, cleanupFunc);
                player.on(VPAID_EVENTS.AdUserClose, cleanupFunc);
                player.on(VPAID_EVENTS.AdError, cleanupFunc);
            ...
            .catch(function (reason) {
                if (reason.message === 'EmptyVast') {
                    this.callEvent_(VPAID_EVENTS.AdError, reason);
                } else {
    ...
    callEvent_: function (eventType, param) {
        ...
        try {
            var eventsCallback = this.eventsCallbacks_[eventType];
            if (eventsCallback) {
                if (param) {
                    eventsCallback(param);
                } else {
                    eventsCallback();
                }
            }
    ...
    getAdRemainingTime: proxyGetter('adRemainingTime', 60, false),
    getAdDuration: proxyGetter('adDuration', 60, false),
