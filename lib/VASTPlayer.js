...
VASTPlayer.prototype.load = function load(uri) {
    ...
    return VAST.fetch(uri, config).then(function loadPlayer(vast) {
        var config = (function() {
            ...
            var linear = vast.filter('ads[0].creatives[0].linear');
            var skipoffset = linear ? linear.skipoffset : null
            ...
            if (jsVPAIDFiles.length > 0) {
                ...
                return {
                    player: new FlashVPAIDPlayer(self.slot, self.videoSlot, VASTPlayer.vpaidSWFLocation),
