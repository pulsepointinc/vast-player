...
function getNotReadyError(name) {
    return new Error('VASTPlayerNotReady:' + name);
}
...
function proxy(method) {
    return function callMethod() {
        ...
        if (!this.ready) {
            return LiePromise.reject(getNotReadyError(method));
        }
...
function proxyProp(property) {
    return {
        get: function get() {
            if (!this.ready) { throw getNotReadyError(property); }
...
function VASTPlayer(slot, videoSlot, config, videoSlotCanAutoplay) {
    var self = this;
    if (!slot || !videoSlot) {
        throw new Error('Both slot and video slot are required');
    }
    EventEmitter.call(this); // call super()
    this.vpaidSWFLocation = null;
    this.__private__ = {
        slot: slot,
        videoSlot: videoSlot,
        videoSlotCanAutoplay: videoSlotCanAutoplay,
...
Object.defineProperties(VASTPlayer.prototype, {
    slot: {
        get: function getSlot() {
            return this.__private__.slot;
        }
    },

    videoSlot: {
        get: function getVideoSlot() {
            return this.__private__.videoSlot;
        }
    },

    videoSlotCanAutoplay: {
        get: function getVideoSlotCanAutoplay() {
            return this.__private__.videoSlotCanAutoplay;
        }
    },
    ...
    adExpanded: proxyProp('adExpanded'),
    adWidth: proxyProp('adWidth'),
    adHeight: proxyProp('adHeight'),
    adSkippableState: proxyProp('adSkippableState'),
    adCompanions: proxyProp('adCompanions'),
    adIcons: proxyProp('adIcons'),
    adLinear: proxyProp('adLinear')
});
...
VASTPlayer.prototype.load = function load(uri) {
    ...
    return VAST.fetch(uri, config).then(function loadPlayer(vast) {
        ...
        var config = (function () {
            ...
            var videoFiles = vast.filter('ads[0].creatives[0].mediaFiles', function (mediaFile) {
                return !mediaFile.apiFramework;
            });
            var creative = vast.get('ads[0].creatives[0]');
            var skipoffset = creative ? creative.skipoffset : null;

            if (jsVPAIDFiles.length > 0) {
                return {
                    player: new JavaScriptVPAIDPlayer(self.slot, self.videoSlot, self.videoSlotCanAutoPlay),
                    mediaFiles: jsVPAIDFiles
                };
            } else if (videoFiles.length > 0) {
                return {
                    player: new HTMLVideoPlayer(self.slot, self.videoSlot, skipoffset),
                    mediaFiles: videoFiles
                };
            } else if (swfVPAIDFiles.length > 0) {
                return {
                    player: new FlashVPAIDPlayer(self.slot, self.videoSlot, self.vpaidSWFLocation),
...
VASTPlayer.prototype.expandAd = proxy('expandAd');
VASTPlayer.prototype.collapseAd = proxy('collapseAd');
VASTPlayer.prototype.resizeAd = proxy('resizeAd');
